...
async function seed() {
  ...
  // 3b. Seed Himachal-themed trek packages (isTrek=true)
  console.log("🥾 Seeding trek packages...");
  for (const trek of TREK_PACKAGES) {
    const payload = {
      title: trek.title,
      ...
      imageUrls: JSON.stringify(trek.imageUrls),
      vehicleType: trek.vehicleType,
      maxOccupancy: trek.maxOccupancy,
      description: trek.description,
      itinerary: JSON.stringify(trek.itinerary),
      inclusions: JSON.stringify(trek.inclusions),
      exclusions: JSON.stringify(trek.exclusions),
      categories: JSON.stringify(trek.categories),
  ...
  // 11. Seed Internal pages / nav groups
  console.log("📑 Seeding nav groups (package + trek types)...");
  const packageGroups = [
  ...
  for (const page of packageGroups) {
  ...
  for (const group of trekGroups) {
    ...
    await prisma.internalPage.upsert({
      ...
      update: {
        ...data,
        destinations: { set: dests.map((d) => ({ id: d.id })) },
      },
      create: {
        ...data,
    ...
    console.log(`   • ${group.title} (${dests.length} dest)`);
  }
